...
SELECT DISTINCT on (date_start)
	...
	ROUND(MTN_LAG_ASR_IN,2) AS MTN_LAG_ASR_IN, ROUND(AIRTEL_LAG_ASR_IN,2) AS AIRTEL_LAG_ASR_IN, ROUND(NINE_MOBILE_LAG_ASR_IN,2) AS NINE_MOBILE_LAG_ASR_IN, 
...
SELECT DISTINCT ON (date_start) date_start, id
	,(COALESCE(MTN_LAG_calls_count,0) + COALESCE(MTN_ABJ_calls_count,0) + COALESCE(MTN_ASABA_ANON_calls_count,0) + COALESCE(MTN_ASABA_calls_count,0)) AS MTN_TOTAL_CALLS_COUNT
	,(COALESCE(AIRTEL_LAG_calls_count,0) + COALESCE(AIRTEL_ABJ_calls_count,0) + COALESCE(AIRTEL_ASABA_ANON_calls_count,0) + COALESCE(AIRTEL_ASABA_calls_count,0)) AS AIRTEL_TOTAL_CALLS_COUNT
	,COALESCE(NINE_MOBILE_LAG_calls_count, 0) AS NINE_MOBILE_TOTAL_CALLS_COUNT
	,COALESCE(GLO_LAG_calls_count, 0) AS GLO_TOTAL_CALLS_COUNT
FROM ASR_IN

--select id, created_at , date_start,  created_at::date - date_start::date  from reports.cdr_custom_report
--WHERE 
--	EXTRACT ('year' FROM date_start)=2022 
--	AND EXTRACT('month' FROM date_start) = 10  --CHANGE MONTH HERE
