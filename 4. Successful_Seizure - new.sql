...
WHERE 
	EXTRACT ('year' FROM date_start)=2022 
	AND EXTRACT('month' FROM date_start) = 12  --CHANGE MONTH HERE
...
SELECT rccrd.id, rccrd.report_id, customer_id, customer_auth_id, agg_calls_count, agg_customer_calls_duration, agg_calls_duration, agg_calls_acd, agg_asr_origination, agg_asr_termination 	
...
MTN_LAG AS (
SELECT report_id, customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS MTN_LAG_calls_count, (agg_asr_origination * 100) AS MTN_LAG_ASR_IN
FROM selected_customer
WHERE customer_auth_id = 20092
),
AIRTEL_LAG AS (
SELECT report_id,customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS AIRTEL_LAG_calls_count, (agg_asr_origination * 100) AS AIRTEL_LAG_ASR_IN
FROM selected_customer
WHERE customer_auth_id = 20095
),
NINE_MOBILE_LAG AS (
SELECT report_id,customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS NINE_MOBILE_LAG_calls_count, (agg_asr_origination * 100) AS NINE_MOBILE_LAG_ASR_IN
FROM selected_customer
WHERE customer_auth_id = 20087
),
GLO_LAG AS (
SELECT report_id,customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS GLO_LAG_calls_count, (agg_asr_origination * 100) AS GLO_LAG_ASR_IN
FROM selected_customer
...
MTN_ABJ AS (
SELECT report_id,customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS MTN_ABJ_calls_count, (agg_asr_origination * 100) AS MTN_ABJ_ASR_IN
FROM selected_customer
WHERE customer_auth_id = 20101
),
AIRTEL_ABJ AS (
SELECT report_id, customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS AIRTEL_ABJ_calls_count, agg_asr_origination * 100 AS AIRTEL_ABJ_ASR_IN
FROM selected_customer
WHERE customer_auth_id = 20102
),
--MTN AND AIRTEL FOR ASABA
MTN_ASABA_ANON AS (
SELECT report_id, COALESCE (agg_calls_count,  0) AS MTN_ASABA_ANON_calls_count, (COALESCE(agg_asr_origination,0)) * 100 AS MTN_ASABA_ANON_ASR_IN
FROM selected_customer
...
SELECT DISTINCT on (date_start)
	...
	ROUND(((MTN_ASABA_ASR_IN*MTN_ASABA_calls_count) + (MTN_ASABA_ANON_ASR_IN * MTN_ASABA_ANON_calls_count))/(MTN_ASABA_calls_count + MTN_ASABA_ANON_calls_count),2)  AS AVG_MTN_ASABA_ASR_IN,
...
SELECT DISTINCT ON (date_start) date_start, id
	,ROUND((MTN_LAG_ASR_IN * MTN_LAG_calls_count)/100) AS MTN_LAG_Successful_seizure 
	,ROUND((AIRTEL_LAG_ASR_IN * MTN_LAG_calls_count)/100) AS AIRTEL_LAG_Successful_seizure
	,ROUND((NINE_MOBILE_LAG_ASR_IN * NINE_MOBILE_LAG_calls_count)/100) AS NINE_MOBILE_LAG_Successful_seizure
	,ROUND((GLO_LAG_ASR_IN * GLO_LAG_calls_count)/100) AS GLO_LAG_Successful_seizure
	
	,ROUND((MTN_ABJ_ASR_IN * MTN_ABJ_calls_count)/100) AS MTN_ABJ_Successful_seizure
	,ROUND((AIRTEL_ABJ_ASR_IN * AIRTEL_ABJ_calls_count)/100) AS AIRTEL_ABJ_Successful_seizure
	,ROUND((COALESCE(MTN_ASABA_ASR_IN,0) *(COALESCE(MTN_ASABA_ANON_calls_count,0) + COALESCE(MTN_ASABA_calls_count,0)))/100) AS AVG_MTN_ASABA_Successful_seizure
	,ROUND((COALESCE(AVG_AIRTEL_ASABA_ASR_IN,0) *(COALESCE(AIRTEL_ASABA_ANON_calls_count,0) + COALESCE(AIRTEL_ASABA_calls_count,0)))/100) AS AVG_AIRTEL_ASABA_Successful_seizure
FROM ASR_IN
