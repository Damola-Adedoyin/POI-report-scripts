...
WHERE 
	...
	AND EXTRACT('day' FROM date_start) BETWEEN 13 AND 17
...
SELECT rccrd.id, rccrd.report_id, customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS agg_calls_count, agg_customer_calls_duration, agg_calls_duration, floor(agg_calls_acd / 60) || 'm : ' || round((agg_calls_acd % 60)) || 's' AS agg_calls_acd, COALESCE (agg_asr_origination,  0) as agg_asr_origination, agg_asr_termination 	
...
MTN_LAG AS (
SELECT report_id,customer_id, customer_auth_id, agg_calls_count AS MTN_LAG_calls_count, (agg_asr_origination * 100) AS MTN_LAG_ASR_IN,  agg_calls_acd AS MTN_LAG_ACD
FROM selected_customer
WHERE customer_auth_id = 20092
),
AIRTEL_LAG AS (
SELECT report_id,customer_id, customer_auth_id, agg_calls_count AS AIRTEL_LAG_calls_count, (agg_asr_origination * 100) AS AIRTEL_LAG_ASR_IN,  agg_calls_acd AS AIRTEL_LAG_ACD
FROM selected_customer
WHERE customer_auth_id = 20095
),
NINE_MOBILE_LAG AS (
SELECT report_id,customer_id, customer_auth_id, agg_calls_count AS NINE_MOBILE_LAG_calls_count, (agg_asr_origination * 100) AS NINE_MOBILE_LAG_ASR_IN,  agg_calls_acd AS NINE_MOBILE_LAG_ACD
FROM selected_customer
...
MTN_ABJ AS (
SELECT report_id,customer_id, customer_auth_id, agg_calls_count AS MTN_ABJ_calls_count, (agg_asr_origination * 100) AS MTN_ABJ_ASR_IN, agg_calls_acd  AS MTN_ABJ_ACD
FROM selected_customer
WHERE customer_auth_id = 20101
),
AIRTEL_ABJ AS (
SELECT report_id, customer_id, customer_auth_id, agg_calls_count AS AIRTEL_ABJ_calls_count, agg_asr_origination * 100 AS AIRTEL_ABJ_ASR_IN,  agg_calls_acd AS AIRTEL_ABJ_ACD
FROM selected_customer
WHERE customer_auth_id = 20102
),
--MTN AND AIRTEL FOR ASABA
MTN_ASABA_ANON AS (
SELECT report_id, COALESCE (agg_calls_count,  0) AS MTN_ASABA_ANON_calls_count, COALESCE ((agg_asr_origination * 100),0)  AS MTN_ASABA_ANON_ASR_IN,  agg_calls_acd AS MTN_ASABA_ANON_ACD
FROM selected_customer
WHERE customer_auth_id = 20151
),

MTN_ASABA AS (
SELECT report_id,customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS MTN_ASABA_calls_count, COALESCE (agg_asr_origination,  0) * 100 AS MTN_ASABA_ASR_IN,  agg_calls_acd AS MTN_ASABA_ACD
FROM selected_customer
WHERE customer_auth_id = 20141
),
AIRTEL_ASABA_ANON AS (
SELECT report_id, COALESCE (agg_calls_count,  0) AS AIRTEL_ASABA_ANON_calls_count, COALESCE (agg_asr_origination,  0) * 100 AS AIRTEL_ASABA_ANON_ASR_IN,  agg_calls_acd AS AIRTEL_ASABA_ANON_ACD
FROM selected_customer
WHERE customer_auth_id = 20150
),
AIRTEL_ASABA AS (
SELECT report_id,customer_id, customer_auth_id, COALESCE (agg_calls_count,  0) AS AIRTEL_ASABA_calls_count, COALESCE (agg_asr_origination,  0) * 100 AS AIRTEL_ASABA_ASR_IN,  agg_calls_acd AS AIRTEL_ASABA_ACD
FROM selected_customer
...
SELECT DISTINCT on (date_start)
	...
	MTN_ABJ_calls_count, AIRTEL_ABJ_calls_count,
	MTN_ASABA_ANON_calls_count, MTN_ASABA_calls_count, AIRTEL_ASABA_ANON_calls_count, AIRTEL_ASABA_calls_count,
	MTN_LAG_ACD,AIRTEL_LAG_ACD, NINE_MOBILE_LAG_ACD, GLO_LAG_ACD, 
	MTN_ABJ_ACD, AIRTEL_ABJ_ACD,
	MTN_ASABA_ACD, MTN_ASABA_ANON_ACD, AIRTEL_ASABA_ACD, AIRTEL_ASABA_ANON_ACD 
FROM MTN_LAG
	LEFT JOIN AIRTEL_LAG ON AIRTEL_LAG.report_id = MTN_LAG.report_id
	LEFT JOIN NINE_MOBILE_LAG ON NINE_MOBILE_LAG.report_id = MTN_LAG.report_id
	LEFT JOIN GLO_LAG ON GLO_LAG.report_id = MTN_LAG.report_id
...
SELECT DISTINCT ON (date_start) date_start, id, 
	MTN_LAG_ACD, AIRTEL_LAG_ACD,NINE_MOBILE_LAG_ACD, GLO_LAG_ACD, 
	MTN_ABJ_ACD,AIRTEL_ABJ_ACD,
	MTN_ASABA_ACD, AIRTEL_ASABA_ACD, MTN_ASABA_ANON_ACD, AIRTEL_ASABA_ANON_ACD 
FROM ACD
