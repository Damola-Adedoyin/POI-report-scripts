...
utilization as (
	select  
	(
		select created_at :: date from orig_table_range 
	) as created_date,
	
	(
		select created_at :: date from term_table_range 
...
--select * from count_orig_term_join



--EXTRACT('day' FROM orig_table_range.calls_time) as orig_date, EXTRACT('day' FROM term_table_range.calls_time)  as term_date,